...
def tags_bar_graph(tag_count, tag_names):
    ...
    fig.update_layout(title="tags_bar_graph")
    fig.show()
...
def authors_pie_chart(authors, values):
    ...
    data=trace
    fig = go.Figure(data=data)
    fig.update_layout(title="Blogs Per Author")
    fig.show()
    return "Opening a new tab in your web browser..."
...
def comments_line_graph(dates, comments):
    ...
    data = [trace0]
    fig = go.Figure(data=data)
    fig.update_layout(title='Comments Per Month')
    fig.show()
...
def images_histogram(blogsites, img_counts):
    ...
    fig = go.Figure(data=data)
    fig.update_layout(title='Images Per Blogsite and Sum')
    fig.show()
    return "Opening a new tab in your web browser..."
...
# Help txt - user interface
def load_help_text():
    with open('help_cmd.txt') as file:
        return file.read()
